...
$selectedTab = isset($_GET['tab']) ? $_GET['tab'] : 'placeOrder';
$joinTable = 'orders';

// Update the query to exclude items with the status 'Received'
$query = "SELECT id, status, cart_data, created_at FROM $joinTable WHERE status != 'Received'";
...
$filteredCartDataList = array_filter($cartDataList, function ($item) use ($selectedTab) {
    return $item['orderStatus'] === 'To Be Delivered' && $selectedTab === 'placeOrder';
...
<div class="container">
        ...
        <button class="btn btn-primary btn-sm received-btn">Received</button>
    </div>
                    </div>
                </div>
...
<script>
    ...
    const myTabs = document.querySelectorAll('#myTabs .nav-item .nav-link');
    myTabs.forEach(tab => {
        tab.addEventListener('click', function () {
            // Remove the 'active' class from all tabs
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const receivedButton = document.querySelector('.received-btn');

        receivedButton.addEventListener('click', function () {
            const orderId = <?= $orderId ?>; // Use the actual order ID here
            const rowToRemove = this.closest('tr');

            // Send an AJAX request to update the order status to "Received" and delete the item
            const updateStatusUrl = 'updateStatus.php';
            fetch(updateStatusUrl, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                },
                body: 'orderId=' + orderId + '&status=Received',
            })
